...
import {
  ...
  Volume2, TrendingUp, ChevronLeft, ChevronRight,
...
const StatCard = ({ icon, label, value }) => (
  ...
  >
    ...
    <p className="text-white font-semibold text-xs sm:text-sm leading-snug break-words">
      {value ?? "—"}
    </p>
  </div>
);
  ...
  return (
    ...
    >
      <Link to={`/anime/${item.mal_id}`}>
        ...
        <p
          className="text-white font-semibold line-clamp-2 leading-tight group-hover:text-indigo-300 transition-colors"
          style={{ fontSize: "clamp(10px,2.5vw,12px)" }}
        >
          {item.title_english || item.title}
        </p>
        {item.year && (
          <p className="mt-0.5" style={{ fontSize: 10, color: MUTED_LO }}>
            {item.year}
          </p>
        )}
      </Link>
    ...
    <div>
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        ...
        <div className="flex gap-1 flex-shrink-0 ml-2">
          {[
            { dir: -1, Icon: ChevronLeft, can: canLeft },
            { dir: 1, Icon: ChevronRight, can: canRight },
          ].map(({ dir, Icon: Ic, can }) => (
            <button
              key={dir}
              onClick={() => scroll(dir)}
      ...
      {!items?.length ? (
        <p className="text-xs sm:text-sm py-4" style={{ color: MUTED_LO }}>
          {emptyMsg}
        </p>
      ) : (
        <div
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p
          className="text-white font-semibold line-clamp-2 leading-snug group-hover:text-indigo-300 transition-colors"
...
const AnimeDetail = () => {
      ...
      .then(async (data) => {
        ...
        if (prevEntry?.mal_id) {
          try {
            ...
            if (!cancelled) setPrev(prevEntry);
          }
        }
        ...
        if (nextEntry?.mal_id) {
          try {
            ...
            if (!cancelled) setNext(nextEntry);
          }
        }
  ...
  const hasTrailer =
    anime.trailer?.youtube_id || anime.trailer?.embed_url || anime.trailer?.url;
  ...
  const statsLeft = [
    {
      icon: <Star size={13} />,
      label: "Score",
      value: anime.score ? `${anime.score.toFixed(2)} / 10` : "N/A",
    },
    {
  ...
  const statsRight = [
    { icon: <Tv size={13} />, label: "Type", value: anime.type },
    {
      icon: <Clock size={13} />,
      label: "Episodes",
      value: anime.episodes ? `${anime.episodes} eps` : null,
    },
        ...
        >
          <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-2 sm:mb-3">
            {anime.genres?.slice(0, 3).map((g) => (
              <Pill key={g.mal_id} color="indigo">
                {g.name}
              </Pill>
            ))}
          ...
          {anime.title_english && anime.title !== anime.title_english && (
            <p
              className="mb-2 sm:mb-3 font-medium"
              style={{ color: "rgba(255,255,255,.35)", fontSize: "clamp(12px,2vw,15px)" }}
            >
              {anime.title}
            </p>
          )}
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            {anime.score && (
              <span className="flex items-center gap-1">
                ...
                <span className="text-xs" style={{ color: "rgba(255,255,255,.28)" }}>
                  / 10
                </span>
              </span>
            )}
            {anime.year && (
              <span className="text-xs sm:text-sm" style={{ color: "rgba(255,255,255,.42)" }}>
                {anime.year}
              </span>
            )}
        ...
        <div className="flex flex-col xl:flex-row gap-6 sm:gap-8 xl:gap-10">
          <div className="xl:w-64 2xl:w-72 flex-shrink-0">
              ...
              {(prevSeason || nextSeason) && (
                ...
                >
                  ...
                  <div className="flex flex-col gap-2">
                    {prevSeason && (
                      <SeasonCard item={prevSeason} label="← Previous" direction="prev" />
                    )}
                    {nextSeason && (
                      <SeasonCard item={nextSeason} label="Next →" direction="next" />
                    )}
                  </div>
                </motion.div>
              )}
            ...
            {audioLangs.length > 0 && (
              ...
              >
                <p
                  className="text-[10px] sm:text-xs uppercase tracking-wider font-semibold mb-2 sm:mb-3"
                  style={{ color: MUTED }}
                >
                  <Volume2 size={10} className="inline mr-1.5 -mt-0.5" />
                  Audio
                </p>
                <div className="flex flex-wrap gap-1.5">
                  {audioLangs.map((l) => (
          ...
          >
            <div
              className="mb-5 sm:mb-7 overflow-x-auto"
              style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}
            >
              <div
                ...
                >
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5 sm:gap-2">
                      ...
                      {anime.themes?.map((t) => (
                        <Pill key={t.mal_id}>{t.name}</Pill>
                      ))}
                      {anime.demographics?.map((d) => (
                        <Pill key={d.mal_id} color="pink">
                          {d.name}
                        </Pill>
                      ))}
                    </div>
                  ...
                  {anime.background && (
                    <div>
                      ...
                      <p
                        className="leading-relaxed"
                        style={{ color: "rgba(148,163,184,.60)", fontSize: "clamp(12px,2vw,14px)" }}
                      >
                        {anime.background}
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                      {anime.studios?.length > 0 && (
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {anime.studios.map((s) => (
                              <Pill key={s.mal_id} color="yellow">
                                {s.name}
                              </Pill>
                            ))}
                          </div>
                      ...
                      {anime.producers?.length > 0 && (
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {anime.producers.slice(0, 5).map((p) => (
                              <Pill key={p.mal_id}>{p.name}</Pill>
                            ))}
                          </div>
                        </div>
                      )}
                      ...
                      <div className="flex items-end justify-between mb-3 sm:mb-4">
                        <div>
                          ...
                          <p className="font-black text-white" style={{ fontSize: "clamp(28px,6vw,52px)" }}>
                            ...
                            <span
                              className="text-base sm:text-xl font-normal ml-1.5"
                  ...
                  {(prevSeason || nextSeason) && (
                    <div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                        {prevSeason && (
                          <SeasonCard item={prevSeason} label="← Previous Season" direction="prev" />
                        )}
                        {nextSeason && (
                          <SeasonCard item={nextSeason} label="Next Season →" direction="next" />
                        )}
                      </div>
                    </div>
                  )}
                      ...
                      {characters.map(({ character, role, voice_actors }, i) => (
                        ...
                        >
                          ...
                          <div className="p-2 sm:p-2.5">
                            ...
                            <p
                              className="mt-0.5"
                              style={{ fontSize: 10, color: role === "Main" ? INDIGO : MUTED_LO }}
                            >
                              {role}
                            </p>
                            {voice_actors?.[0] && (
                              <p
                                ...
                                style={{ fontSize: 9, color: "rgba(148,163,184,.35)" }}
                              >
              ...
              {activeTab === "details" && (
                ...
                >
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
                    {statsRight.filter((s) => s.value).map((s) => (
                      <StatCard key={s.label} {...s} />
                    ))}
                  </div>
